...
int pwmCommandFromSpeed(float desiredSpeed_rad_per_sec) {
  ...
  float a = -69.3f;  // the zeroth order term
  float b = 0.145f;  // the first order term
  float pwmOutput = 0;
  if (desiredSpeed_rad_per_sec > (-a/b)) {
    pwmOutput = a + b * desiredSpeed_rad_per_sec;
  }

  return int(pwmOutput);
}
...
float speedFromForce(float desiredForce_N) {
  ...
  // precision!
  float const propConstant = 3.8569e-8f; // calculated through quadratic fit in Desmos
